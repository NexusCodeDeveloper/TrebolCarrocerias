...
export default function Testimonios() {
  ...
  return (
    <section id="testimonios" className="py-32 md:py-48 px-4 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto relative z-10" ref={ref}>
        ...
        >
          <span className="text-trebol-400 text-sm font-medium tracking-[0.3em] uppercase mb-6 block">Confianza</span>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-6 md:gap-8">
          {clients.map((client, i) => (
            ...
            >
              ...
              <div className={`w-20 h-20 rounded-2xl bg-gradient-to-br ${client.color} flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform duration-500`}>
                <span className="text-white font-heading font-bold text-3xl">{client.name[0]}</span>
              </div>

              {/* Client name */}
              <div className="absolute bottom-4 left-0 right-0 text-center">
                <span className="text-gray-500 text-sm font-heading font-bold tracking-tight">{client.name}</span>
              </div>
            </motion.div>
          ))}
        </div>
        ...
        >
          Reemplazá estos placeholders con los logos reales de cada empresa
        </motion.p>
      </div>
    </section>
  )
